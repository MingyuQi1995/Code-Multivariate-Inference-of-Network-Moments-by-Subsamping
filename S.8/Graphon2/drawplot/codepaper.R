...
load("/Users/mingyuqi/Desktop/netsimures/graphon2/b06/simu05/eval.Rdata")
setwd("/Users/mingyuqi/Desktop/netsimures/graphon2/b06/simu05")
...
p1<-ggplot(d1, aes(x=g, y=Mean, colour=variable, group=variable)) +
  ...
  scale_y_continuous(limits = c(0.05, 0.48),
                     breaks=c(0.10,0.21,0.32,0.43),
                     labels = c("0.10", "0.21", "0.32", "0.43"),
...
p3 <- ggplot(d1, aes(x=g, y= log(Mean), colour=variable, group=variable)) +
  ...
  theme(panel.grid=element_blank(),
        ...
        axis.title.y = element_text(size = bs))+
  scale_x_discrete(expand=expansion(add=0.4))+
  scale_y_continuous(limits = c(-2.6, -0.3),
...
p2<-ggplot(d1, aes(x=g, y=Mean, colour=variable, group=variable)) +
  ...
  theme(panel.grid=element_blank(),
        ...
        axis.title.y = element_text(size = bs))+
  scale_x_discrete(expand=expansion(add=0.4))+
  scale_y_continuous(limits = c(0.09, 0.44),
                     breaks=c(0.10,0.21,0.32,0.43),
                     labels = c("0.10", "0.21", "0.32", "0.43"),
                     expand=expansion(add=0.02)) +
